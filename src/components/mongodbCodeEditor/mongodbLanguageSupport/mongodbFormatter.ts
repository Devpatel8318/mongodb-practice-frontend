// Function to format MongoDB queries
...
const formatStage = (stageStr: string): string => {
	try {
		...
		const formattedValue = formatStageValue(valueStr, true)
	...
	try {
		...
		if (valueStr.startsWith('{') && valueStr.endsWith('}')) {
			...
			const formattedPairs = pairs.map(({ key, value, comment }) => {
				// Check if the value is an array that needs special formatting
				const needsMultilineArray =
					value.startsWith('[') &&
					value.endsWith(']') &&
					shouldFormatAsMultilineArray(value)

				// Format the value recursively
				const formattedValue = needsMultilineArray
					? formatArrayMultiline(value, 8) // Deeper indent for nested arrays
					: formatValueWithIndentation(value, isRoot ? 6 : 8)
		...
		// Format arrays
		if (valueStr.startsWith('[') && valueStr.endsWith(']')) {
			if (shouldFormatAsMultilineArray(valueStr)) {
				return formatArrayMultiline(valueStr, isRoot ? 6 : 8)
			}
			...
			const formattedItems = items.map((item) =>
				formatValueWithIndentation(item, isRoot ? 6 : 8)
			)
			return `[\n      ${formattedItems.join(',\n      ')}\n    ]`
		}
...
// Determine if an array should be formatted as multiline
const shouldFormatAsMultilineArray = (arrayStr: string): boolean => {
	const innerContent = arrayStr.substring(1, arrayStr.length - 1).trim()
	if (!innerContent) return false

	const items = parseArrayItems(innerContent)

	// Format as multiline if:
	// 1. Array has more than 3 items, or
	// 2. Any item contains an object or another array, or
	// 3. The whole array is longer than 60 characters
	return (
		items.length > 3 ||
		items.some(
			(item) =>
...
// Format an array with proper multiline indentation
const formatArrayMultiline = (
	arrayStr: string,
	indentLevel: number
): string => {
	const innerContent = arrayStr.substring(1, arrayStr.length - 1).trim()
	...
	const formattedItems = items.map((item) => {
		// If item is an object or array, format recursively
		if (
			(item.includes('{') && item.includes('}')) ||
			(item.includes('[') && item.includes(']'))
		) {
			return formatValueWithIndentation(item, indentLevel + 2)
		}
		return item
	})

	return `[\n${innerIndent}${formattedItems.join(',\n' + innerIndent)}\n${indent}]`
}
...
const formatValueWithIndentation = (
	...
	if (value.startsWith('[') && value.endsWith(']')) {
		...
		// Check if this array should be formatted as multiline
		if (shouldFormatAsMultilineArray(value)) {
			return formatArrayMultiline(value, indentLevel)
		}
